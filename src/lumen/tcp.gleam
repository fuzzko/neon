...
) -> Result(Tcp, net.PosixError) {
  host
  |> charlist.from_string
  |> tcp_connect_(port, ip_version)
}

pub fn send(socket: Tcp, payload: BitArray) -> Result(Tcp, net.PosixError) {
...
pub fn shutdown(socket: Tcp) -> Result(Nil, net.PosixError) {
...
pub fn listen(opts: ListenOptions) -> Result(Tcp, net.PosixError) {
  tcp_listen_(opts)
}

pub fn accept(socket: Tcp, timeout: Int) -> Result(Tcp, net.PosixError) {
...
fn tcp_send_(socket: Tcp, packet: BitArray) -> Result(Nil, net.PosixError)

@external(erlang, "lumen_ffi", "tcp_shutdown")
fn tcp_shutdown_(socket: Tcp) -> Result(Nil, net.PosixError)

@external(erlang, "lumen_ffi", "tcp_listen")
fn tcp_listen_(opts: ListenOptions) -> Result(Tcp, net.PosixError)

@external(erlang, "lumen_ffi", "tcp_accept")
fn tcp_accept_(listener: Tcp, timeout: Int) -> Result(Tcp, net.PosixError)
